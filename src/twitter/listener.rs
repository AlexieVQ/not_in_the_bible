...
/// request queue.
///
/// Only handles requests sent after a given date, if provided.
pub fn listen(connection: &Connection,
    ...
    loop {
        ...
        for mention in mentions {
            if since.map_or_else(|| true,
                |d| mention.parse_date() >= d) {
                let mut _st: Option<Tweet> = None;
                let mut quoted = false;
                let op: Option<&Tweet> = match mention.in_reply_to_status_id_str
                    .as_ref() {
                    Some(op_id) => {
                ...
                trace!("Tweet {} created before {}, ignored.", mention.id_str,
                    since.unwrap());
            }
        }
